...
func Elaborate(input *os.File, output *os.File, cache *Cache) error {
	...
	for scanner.Scan() {
		...
		case "JOBEND":
			content, job, err = formatJob(data)
		case "LOGIN":
			content, log, err = formatLog(data)
			if err == nil {
				cache.Log = log
			}
		case "LOGOFF":
		...
		// Elaborate interval if process end is recognized
		if data[1] == "JOBEND" && cache.Job != nil && job != nil && cache.Job.Value == job.Value {
			...
			fmt.Fprintf(output, "%s\n", string(jsonByte))
		} else if data[1] == "LOGOFF" && cache.Log != nil && log != nil && cache.Log.Value == log.Value {
			content = formatLogInterval(*cache.Log, *log)
			jsonByte, err = json.Marshal(content)
			if err != nil {
				return err
			}
...
func formatWater(data []string) (*model.DataContent, error) {
	if len(data) < 1 {
...
func formatLog(data []string) (*model.DataContent, *model.DataGauge, error) {
	if len(data) < 2 {
		return nil, nil, fmt.Errorf("invalid %s data record", data[1])
	}
...
func formatLogInterval(jobStart model.DataGauge, jobEnd model.DataGauge) *model.DataContent {
	m := model.DataInterval{
		Start: jobStart.Timestamp,
		End:   jobEnd.Timestamp,
		Key:   jobStart.Key,
